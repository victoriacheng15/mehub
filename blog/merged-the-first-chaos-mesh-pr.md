...
Certain trade-offs exist in this initial release. Namespace management and cleanup are confined to the local `TestContext`, and the main `make e2e` target does not invoke the `Gherkin` suite. Addressing these gaps provides a clear objective for the subsequent pull request.
